...
parseLogActivity = LogActivity <$> parseLogTime <*> (Title <$> (spaces *> manyTill anyChar (try $ many (char ' ') <* (string "--" <* many (char ' ')) <|> (pure <$> newline)))) <*> ((fmap . fmap) LogComment (optional $ some $ notChar '\n'))
...
parseok = parseString (manyTill anyChar (try $ many (char ' ') <* (string "--" <* many (char ' ')) <|> (pure <$> newline))) mempty "hello --\n"
